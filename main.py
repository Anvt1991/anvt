...
"""
Bot Chứng Khoán Toàn Diện Phiên Bản V18.8 (Tối ưu hóa):
...
# Lưu thời gian bắt đầu để tính uptime
...
async def is_user_approved(user_id) -> bool:
    """Wrapper cho db.is_user_approved"""
...
class AIAnalyzer:
    ...
    def __init__(self):
        self.db = db  # Sử dụng biến db toàn cục
    ...
    async def load_report_history(self, symbol: str) -> list:
        """Wrapper cho db.load_report_history"""
        return await self.db.load_report_history(symbol)
    
    async def save_report_history(self, symbol: str, report: str, close_today: float, close_yesterday: float) -> None:
        """Wrapper cho db.save_report_history"""
        await self.db.save_report_history(symbol, report, close_today, close_yesterday)
...
async def main():
    ...
    try:
        ...
        # Đảm bảo các managers đã được khởi tạo
        global db, redis_manager, model_db_manager
        if 'db' not in globals() or db is None:
            db = DBManager()
        if 'redis_manager' not in globals() or redis_manager is None:
            redis_manager = RedisManager()
        if 'model_db_manager' not in globals() or model_db_manager is None:
            model_db_manager = ModelDBManager()
        ...
        if use_webhook:
            ...
            async def health_check_handler(request):
                ...
                try:
                    ...
                    # Kiểm tra DB
                    db_health = await db.is_user_approved("admin")
...
if __name__ == "__main__":
    # Thời gian bắt đầu đã được khởi tạo ở đầu file
